...
const socket = require('socket.io')(httpserver, {
  ...
  cors :
      {origin : "https://drawersoc.herokuapp.com/", method : [ "GET", "POST" ]}
...
socket.on('connect', (sockt) => {
  ...
  sockt.on('startgame', val => {
    socket.in(sockt.room_name)
                 .emit('ready', Allcon.rooms.get(sockt.room_name).TurnCount);
    //   sockt.to(sockt.room_name).emit('ready',
  ...
  sockt.on('ipoint', (p) => {
    Allcon.rooms.get(sockt.room_name).TurnCount =
        (Allcon.rooms.get(sockt.room_name).TurnCount + 1) %
        Allcon.rooms.get(sockt.room_name).users.length;
  ...
  sockt.on('mssg', (info) => {
    if (info.msg === Allcon.rooms.get(sockt.room_name).currentword) {
      ...
      socket.sockets.in(info.room_name).emit('message', {
        name : info.user_name,
